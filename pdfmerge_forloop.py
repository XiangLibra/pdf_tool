...
def merge_pdfs(input_pdf_paths, page_ranges,num_name):
    ...
    output_pdf_path = os.path.join(os.path.dirname(input_pdf_paths[0]), base_name + str(num_name) +"_merged_out.pdf")
...
# 指定要合併的輸入 PDF 檔案路徑列表
input_pdf_paths = [
    r"D:\Dataset\CPR\胸痛文件\掃描檔\人體同意書簽名檔 (2).pdf",
...
with open(pdf_path, 'rb') as pdf_file:
    ...
    for i in range(0, page_count, 2):
        ...
        # 執行函式以合併指定的 PDF 檔案
        merge_pdfs(input_pdf_paths, page_ranges,i+1)
...
#     for i in range(0, page_count, 2):
#         page_ranges = [
#             # (0, i+1, i+2),  # 人體同意書簽名檔 (1) 的第 1 到第 3 頁
#             # (1, 0, 4),      # 同意書中間內容的第 1 到第 4 頁
#             # (2, i+1, i+2),  # 人體同意書簽名檔 (1) 的第 2 到第 3 頁
#             (i, i+1),  # 只添加第一個 PDF 的第一頁
#             (0, 4),  # 添加第二個 PDF 的第 1 到第 2 頁
#             (i+1, i+2),  # 添加第三個 PDF 的第二頁
#         ]
